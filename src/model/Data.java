...
public class Data implements Savable {
	...
	private void loadGruppenFromFile() {
		...
		try {
			reader = FileUtils.getReaderForFile(new File(this.gruppenfile));
	...
	private void loadPersonenFromFile() {
		BufferedReader reader;
		try {
			reader = FileUtils.getReaderForFile(new File(this.personenfile));
		} catch (FileNotFoundException e1) {
	...
	public void saveAll() {
		...
		Date date = new Date();
		FileUtils.writeToFile(new File(Main.dataFolder + "letzte änderung.txt"),
	...
	private void savePersonenToFile() {
		saveListToFile(this.personenfile, this.personen, new String[] { "vorname",
			"name", "tel" });
	}

	private void saveListToFile(String filename, List<? extends Serializable> liste,
			String[] order) {
		BufferedWriter writer;
		try {
			FileUtils.checkOrCreateDir(new File(Main.dataFolder));
			writer = FileUtils.getWriterForFile(new File(filename));
		} catch (IOException e) {
			Main.debugger.debug("Daten konnten nicht geschrieben werden. " + e
				+ "\nDateiname: " + filename);
			return;
		}
